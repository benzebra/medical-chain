...
const app = express();
const PORT = process.env.PORT || 3001; // Porta diversa da lite-server
...
// GET - Ottenere tutti gli oggetti (compatibile con objects.json)
app.get('/api/objects', async (req, res) => {
  try {
    ...
    else query.status = 'active'; // Solo oggetti attivi di default
    
    const objects = await MedicalObject.find(query).sort({ id: 1 });
    
    // Formato compatibile con il frontend esistente
